...
import { TelegramHandlerContext, ChannelMetadata } from "../types/telegram.types";
...
export class CallbackHandler {
  ...
  async handleCallbackQuery(bot: TelegramBot, callbackQuery: TelegramBot.CallbackQuery): Promise<void> {
    ...
    this.logger.debug(`Received callback query: ${data} from user: ${telegramUser.id}`);
    ...
    try {
      ...
      await this.telegramApiService.sendMessage(bot, chatId, "❌ An error occurred. Please try again.");
    }
  }

  private async routeCallback(bot: TelegramBot, context: TelegramHandlerContext, data: string): Promise<void> {
  ...
  private async handleDynamicCallbacks(bot: TelegramBot, context: TelegramHandlerContext, data: string): Promise<void> {
    if (data.startsWith("manage_channel_")) {
      ...
      await this.telegramApiService.sendMessage(bot, context.chatId, "❌ Unknown action");
    }
  }

  private async handleChannelManagement(bot: TelegramBot, context: TelegramHandlerContext, channelId: string): Promise<void> {
    const { chatId, telegramUser } = context;

    try {
      const user = await this.userManagementService.findUserByTelegramId(telegramUser.id.toString());
      if (!user) {
        await this.telegramApiService.sendMessage(bot, chatId, "❌ User not found.");
        return;
      }

      const channel = await this.channelManagementService.findChannelById(channelId, user.id);
      if (!channel) {
        await this.telegramApiService.sendMessage(bot, chatId, "❌ Channel not found.");
        return;
      }
...
${channel.isActive ? '🟢 Active' : '🔴 Inactive'}`;
      ...
      const keyboard = [
        [
          { text: channel.isActive ? "🔴 Deactivate" : "🟢 Activate", callback_data: `toggle_channel_${channelId}` },
      ...
      await this.telegramApiService.sendMessage(bot, chatId, "❌ Error managing channel.");
    }
  }

  private async handleToggleChannel(bot: TelegramBot, context: TelegramHandlerContext, channelId: string): Promise<void> {
    const { chatId, telegramUser } = context;
    ...
    try {
      ...
      const updatedChannel = await this.channelManagementService.toggleChannelStatus(channelId);
      ...
      await this.telegramApiService.sendMessage(bot, chatId, "❌ Error updating channel status.");
    }
  }

  private async handleRemoveChannel(bot: TelegramBot, context: TelegramHandlerContext, channelId: string): Promise<void> {
    const { chatId, telegramUser } = context;
...
⚠️ This action cannot be undone. You'll need to add the channel again if you want to use it for broadcasting.`;
      ...
      const keyboard = [
        [
          { text: "🗑️ Yes, Remove", callback_data: `confirm_remove_${channelId}` },
      ...
      await this.telegramApiService.sendMessage(bot, chatId, "❌ Error removing channel.");
    }
  }

  private async handleConfirmRemoveChannel(bot: TelegramBot, context: TelegramHandlerContext, channelId: string): Promise<void> {
    const { chatId, telegramUser } = context;
...
You can add it back anytime by adding the bot to the channel again or using the manual addition method.`;
  ...
  private async handleRefreshChannel(bot: TelegramBot, context: TelegramHandlerContext, channelId: string): Promise<void> {
    const { chatId, telegramUser } = context;
    ...
    try {
      ...
      await this.telegramApiService.sendMessage(bot, chatId, "🔄 Refreshing channel information...");

      try {
        // Get updated channel info from Telegram
        const chat = await this.telegramApiService.getChatInfo(bot, parseInt(channel.platformId));
        
        // Check bot permissions
        const { canPost } = await this.telegramApiService.getBotPermissions(bot, chat.id);
        ...
        const updatedChannel = await this.channelManagementService.updateChannel(channelId, {
...
Information has been refreshed successfully!`;
        ...
        const keyboard = [
          [{ text: "🗑️ Remove Channel", callback_data: `remove_channel_${channelId}` }],
          [{ text: "📋 Back to Channels", callback_data: "channels_list" }],
        ];

        await this.telegramApiService.sendMessage(bot, chatId, `❌ Could not refresh channel information. The channel might be inaccessible or the bot might have been removed.
...
You may want to remove this channel and add it again.`, {
      ...
      await this.telegramApiService.sendMessage(bot, chatId, "❌ Error refreshing channel information.");
    }
  }
} 